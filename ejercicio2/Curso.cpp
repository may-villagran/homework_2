...
void Curso::encontrar_estudiante(shared_ptr<Estudiante> estudiante, int &pos){
    for (int i = 0; i < listado_estudiantes.size(); i++) {
        if (listado_estudiantes[i]->getLegajo() == estudiante->getLegajo()) {
...
void Curso::incribir_estudiante(shared_ptr<Estudiante> estudiante){
    ...
    listado_estudiantes.push_back(estudiante);
}
...
void Curso::imprimir_listado(){
    ...
    cout << "Listado de alumnas inscriptos al curso: " << nombre_curso << endl;
    for (const shared_ptr<Estudiante>& estudiante : listado_estudiantes) {
